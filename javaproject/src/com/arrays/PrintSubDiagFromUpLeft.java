...
public class PrintSubDiagFromUpLeft {
	...
	public static void main(String[] args) {
		...
		for(int k=0;k<n;k++) {   //n lines
			...
			int i=k;//every row k starts increasing from 0 to n-1 -> k -> (row by row)
		...
		for(int k=0;k<n-1;k++) { //  n-1 lines remaining
			...
			int j=k+1;//every row k starts increasing from 1 to 3 -> k+1 ->  (row by row)
