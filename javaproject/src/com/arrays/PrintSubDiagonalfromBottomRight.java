...
public class PrintSubDiagonalfromBottomRight {
	...
	public static void main(String[] args) {
		...
		for(int k=0;k<n;k++) {   //n lines
			int i=n-1;//every row i starts with n-1.
			int j=n-1-k;//every row j starts decreasing from n-1 to 0 (row by row)
		...
		for(int k=0;k<n-1;k++) { //  n-1 lines remaining
			int j=0;//every row j starts with 0.
			int i=n-2-k;//every row j starts decreasing from n-2 to 0 (row by row)
